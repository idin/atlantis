...
def knowledge_graph():
    ...
    graph_backend = SimpleGraphBackend()
    return KnowledgeGraphDB(database=graph_db, graph_backend=graph_backend)


def test_add_fact_to_db(knowledge_graph):
    fact_id, original_timestamp = knowledge_graph.add_fact_to_db("Earth", "is_a", "Planet", source="NASA")
...
def test_add_fact_to_graph(knowledge_graph):
    timestamp = knowledge_graph.add_fact_to_graph("Alice", "knows", "Bob", fact_id='f1', source="SocialNetwork", timestamp=datetime.now())
...
def test_get_facts_with_filters(knowledge_graph):
    ts1 = knowledge_graph.add_fact("Earth", "is_a", "Planet", source="NASA")
    time.sleep(0.001)
    ts2 = knowledge_graph.add_fact("Pluto", "is_a", "Dwarf Planet", source="IAU")
...
def test_remove_fact(knowledge_graph):
    source, timestamp = knowledge_graph.add_fact("Venus", "is_a", "Planet", source="NASA")
    fact_id = knowledge_graph.generate_fact_id(subject="Venus", predicate="is_a", obj="Planet", source="NASA")
